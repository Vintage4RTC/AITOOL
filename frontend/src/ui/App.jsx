...
import AutomationScriptsSection from './AutomationScriptsSection.jsx'
    ...
    <div className="min-h-screen relative">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <main role="tabpanel" aria-labelledby={`${tab}-tab`}>
          {tab === 'jira' ? <JiraSection /> : tab === 'healing' ? <HealingDashboard /> : tab === 'automation' ? <AutomationScriptsSection /> : tab === 'jenkins' ? <JenkinsSection /> : <UploadSection />}
        </main>
      </div>
    </div>
  )
...
function UploadSection() {
  ...
  const downloadTestCases = () => {
    ...
    const csvData = generatedTestCases.map((tc, index) => ({
      'Test Name': tc.title || `Test Case ${index + 1}`,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-br from-white/95 via-slate-50/90 to-blue-50/95 backdrop-blur-sm rounded-xl shadow-lg border border-white/30 p-6">
        <h2 className="text-2xl font-bold text-slate-800 mb-2">📁 Upload & Generate Tests</h2>
        <p className="text-slate-700 font-semibold">Upload screenshots or spreadsheets to generate test cases and Playwright scripts</p>
      </div>
      ...
      {/* Generated Results */}
      {(generatedTestCases.length > 0 || generatedScript) && (
          ...
          {generatedTestCases.length > 0 && (
            <div className="mb-8">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-xl font-bold text-slate-900">Generated Test Cases</h3>
                <button
                  onClick={downloadTestCases}
                  className="px-4 py-2 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white text-sm font-medium rounded-lg transition-all duration-200 flex items-center gap-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 shadow-md shadow-blue-500/25 hover:shadow-lg hover:shadow-blue-500/30"
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                  </svg>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {generatedTestCases.map((testCase, index) => (
                  <div key={index} className="p-4 bg-gradient-to-r from-slate-100/90 to-white/90 rounded-lg border border-slate-300/60 shadow-sm hover:shadow-md transition-shadow duration-200">
                    <h4 className="text-lg font-semibold text-slate-900 mb-2">{testCase.title}</h4>
                    <p className="text-slate-800 font-semibold mb-3">{testCase.description}</p>
                    <div className="space-y-2">
                      <div>
                        <span className="text-sm font-bold text-slate-900">Steps:</span>
                        <ul className="mt-1 text-sm text-slate-800 font-semibold">
                          {testCase.steps?.map((step, stepIndex) => (
                            <li key={stepIndex} className="ml-4 list-disc">{step}</li>
                          ))}
                        </ul>
                      </div>
                      {testCase.expectedResult && (
                        <div>
                          <span className="text-sm font-bold text-slate-900">Expected Result:</span>
                          <p className="text-sm text-slate-800 font-semibold">{testCase.expectedResult}</p>
                        </div>
                      )}
                    </div>
                  </div>
